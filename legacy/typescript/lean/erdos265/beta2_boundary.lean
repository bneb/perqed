...
theorem R₁_is_pos_integer (a : ℕ → ℕ) (p : ℤ) (q : ℕ) (hq : q > 0)
  ...
  have h_tail_sum : HasSum (fun k => (1 : ℝ) / (a (k + N) : ℝ)) 
    ...
    exact hasSum_nat_shift h_sum N
  ...
  · rw [h_eq_int]; push_cast [Int.toNat_of_nonneg (le_of_lt h_int_pos)]
...
theorem R₁_eventually_constant (a : ℕ → ℕ) (p : ℤ) (q : ℕ)
  ...
  have h_nat_mono : ∀ N, R₁_nat a p q (N + 1) ≤ R₁_nat a p q N := by
    intro N
    -- R₁(N+1) ≤ R₁(N) in ℝ, and both equal their integer versions cast to ℝ
    have h_mono_N := h_mono_real N
    have h_eq_N := R₁_eq_R₁_int a p q N h_pos_gt0 hq
    have h_eq_N1 := R₁_eq_R₁_int a p q (N + 1) h_pos_gt0 hq
    -- So R₁_int(N+1) ≤ R₁_int(N) in ℤ
    have h_int_mono : R₁_int a p q (N + 1) ≤ R₁_int a p q N := by
      exact_mod_cast (h_eq_N1 ▸ h_eq_N ▸ h_mono_N : (R₁_int a p q (N + 1) : ℝ) ≤ R₁_int a p q N)
    -- And toNat preserves ≤
    exact Int.toNat_le_toNat h_int_mono
  rcases nat_decreasing_eventually_constant (R₁_nat a p q) h_nat_mono with ⟨N₀, hN₀⟩
  use N₀
  intro N hN
  have h_nat_eq := hN₀ N hN
  -- R₁_nat(N+1) = R₁_nat(N) implies R₁(N+1) = R₁(N) via the integer bridge
  have h_eq_N := R₁_eq_R₁_int a p q N h_pos_gt0 hq
  have h_eq_N1 := R₁_eq_R₁_int a p q (N + 1) h_pos_gt0 hq
  rw [h_eq_N, h_eq_N1]
  congr 1
  -- R₁_int = Int.ofNat ∘ R₁_nat (since R₁_int > 0)
  -- h_nat_eq : R₁_nat(N+1) = R₁_nat(N), so R₁_int(N+1) = R₁_int(N)
  unfold R₁_nat at h_nat_eq
  omega
...
theorem shifted_sum_irrational_of_eventually_sylvester (a : ℕ → ℕ)
  ...
  have h_tail_rat : ∃ q : ℚ, ↑q = (↑r : ℝ) - ∑ i ∈ Finset.range N₀, 1 / ((a i : ℝ) - 1) := by
    -- r is rational, finite sum of rationals is rational
    have h_fin_rat : ∃ s : ℚ, (↑s : ℝ) = ∑ i ∈ Finset.range N₀, 1 / ((a i : ℝ) - 1) := by
      refine ⟨∑ i ∈ Finset.range N₀, (1 : ℚ) / ((a i : ℚ) - 1), ?_⟩
      push_cast
      rfl
    rcases h_fin_rat with ⟨s, hs⟩
    exact ⟨r - s, by push_cast; linarith⟩
