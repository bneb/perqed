...
theorem erdos_265_core (a : ℕ → ℕ) (L : ℝ) (hL : L > 1) (N : ℕ)
    (h_res_a : ∀ n ≥ N, a (n + 1) + a n = a n * a n + 1)
    (h_res_b : ∀ n ≥ N, a (n + 1) + 3 * a n = a n * a n + 4) :
    False := by
  have h1N := h_res_a N (le_refl N)
  have h2N := h_res_b N (le_refl N)
