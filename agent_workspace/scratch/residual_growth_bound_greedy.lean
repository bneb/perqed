...
lemma tailResidual_eventually_nonincreasing (seq : ℕ → ℕ) (num denom : ℕ) 
  ...
  use 0
  intro n _
  -- We prove T_{n+1} <= T_n by showing X_n = 1/(seq n - 1) - S_n >= 0, where S_n is the tail sum.
  -- But we can just define X_n = 1/(seq n - 1) - T_n / (denom * P_n).
  -- Wait, let's just use the X_n lemma on X n = 1/(seq n - 1) - S_n.
  sorry
...
theorem greedy_forces_sylvester_recurrence (seq : ℕ → ℕ) (q : ℚ) 
  ...
  have hSum_nd : HasSum (fun k => (1 : ℝ) / (seq k : ℝ)) ((q.num.toNat : ℝ) / q.den) := by
    ...
    rw [hq_eq] at hSum
    exact hSum
  -- Step 2: T_n is non-increasing
  have hNonincr := tailResidual_eventually_nonincreasing seq q.num.toNat q.den 
                     hGe2 q.pos hSum_nd hGreedy
  -- Step 3: Eventually non-increasing + positive → eventually constant
  ...
  -- Step 4: C ≠ 0 and derive the recurrence
